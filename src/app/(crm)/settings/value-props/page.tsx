...
export default async function ValuePropsPage({
  ...
  const [{ error, message }, profile] = await Promise.all([
    searchParams,
    getCurrentProfile(),
  ]);
  const canManageValueProps =
    profile.status === "active" && profile.can_manage_value_props;
  const valueProps = canManageValueProps ? await listValuePropsForSettings() : [];
  ...
  return (
    <section className="space-y-6">
      ...
      {!canManageValueProps ? (
        <Card>
          <CardHeader>
            <CardTitle>Berechtigung erforderlich</CardTitle>
            <CardDescription>
              Dein Profil braucht in der User Verwaltung den Haken bei Value Props.
            </CardDescription>
          </CardHeader>
        </Card>
      ) : null}
